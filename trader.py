...
class Trader:
    ...
    def get_status(self, caller_id):
        ...
        {self.build_key_value('First Trade Time', self.first_trade_time)}\
        {self.build_key_value('Last Trade Time', self.last_trade_time)}\
        {self.build_key_value('16hrs Average Trend Strength Percentage', f'{round(self.avg_trend_strength, 2)}%')}\n\
        {self.build_key_value('', '===== <b>Logs For Admin</b> =====<b>:</b>', False, caller_id)}\n\
    ...
    def update(self, margin_pct, leverage):
        ...
        self.feedback = f'✅ <b>{self.name}</b> trade was successfully update. The trading bot will start using the settings on the next trade action.'

    def stop(self):
        self.alive = False
        self.feedback = f'✅ <b>{self.name}</b> trade was successfully canceled with all BTC sold into the USDT stable coin at market price. \n\nTo add it again or another trade, use the <a href="/{Constants.Commands.addtrade}">/{Constants.Commands.addtrade}</a> command.'
    ...
    def logic(self, data, adx_avg) -> Position:
        ...
        if data['adx'] >= 15:
            ...
            if data['supertrend_is_uptrend_10'] is True \
            and data['supertrend_vwc_10'] > data['supertrend_trend_10'] \
            and data['supertrend_is_uptrend_14'] is True \
            and data['supertrend_vwc_14'] > data['supertrend_trend_14'] \
            and data['supertrend_is_uptrend_15'] is True \
            ...
            # if is downtrend
            elif data['supertrend_is_uptrend_10'] is False \
            and data['supertrend_vwc_10'] < data['supertrend_trend_10'] \
            and data['supertrend_is_uptrend_14'] is False \
            and data['supertrend_vwc_14'] < data['supertrend_trend_14'] \
            and data['supertrend_is_uptrend_15'] is False \
            and data['supertrend_vwc_15'] < data['supertrend_trend_15'] \
            and data['close'] < data['vwap']:
                # Position variables
                order_type = 'sell'
    ...
    def calculate_volume(self):
        return (self.margin_pct * self.pnl) / 100